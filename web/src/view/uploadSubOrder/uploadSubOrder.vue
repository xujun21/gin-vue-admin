<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="商品代码">
         <el-input v-model="searchInfo.productCode" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="商品代码" prop="productCode" width="120" />
        <el-table-column align="left" label="商品中文名" prop="productNameCn" width="120" />
        <el-table-column align="left" label="商品英文名" prop="productNameEn" width="120" />
        <el-table-column align="left" label="包装规格" prop="package" width="120" />
         <el-table-column align="left" label="保质期" width="180">
            <template #default="scope">{{ formatDate(scope.row.expDate) }}</template>
         </el-table-column>
        <el-table-column align="left" label="数量" prop="quantity" width="120" />
        <el-table-column align="left" label="剩余库存" prop="store" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateUploadSubOrderFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="保质期:"  prop="expDate" >
          <el-date-picker v-model="formData.expDate" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
...
import {
  ...
  deleteUploadSubOrder,
  deleteUploadSubOrderByIds,
...
const getTableData = async() => {
  const table = await getUploadSubOrderList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteUploadSubOrderByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUploadSubOrderFunc = async(row) => {
    const res = await findUploadSubOrder({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUploadSubOrderFunc = async (row) => {
    const res = await deleteUploadSubOrder({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
